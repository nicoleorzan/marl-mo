...
from algos.MOReinforce_SER import MOReinforce
import numpy as np
import torch
import wandb
    ...
    for i_batch in range(batch_s):
        ...
        for i in range(config.num_game_iterations):
            ...
            if (_eval == False):
                # save iteration
                for idx_agent, agent in active_agents.items():
                    agent.append_to_replay(i_batch, states[idx_agent], actions[idx_agent], rewards[idx_agent], next_states[idx_agent], logprobs[idx_agent], done)
